use crate::connection::{Connection, ConnectionsManager};
...
impl MirrorServer {
    ...
    pub async fn listen() {
        ...
        //                 MirrorServer::on_connected(callback.connection_id).await;
        //             }
        //             kcp2k_rust::kcp2k_callback::CallbackType::OnData => {
        //                 // MirrorServer::on_data(callback.connection_id, callback.data.clone(), callback.channel.clone()).await;
        //             }
        //             kcp2k_rust::kcp2k_callback::CallbackType::OnError => {
        //                 MirrorServer::on_error(callback.connection_id, callback.error_code.clone(), callback.error_message.clone()).await;
        //             }
        //             kcp2k_rust::kcp2k_callback::CallbackType::OnDisconnected => {
        //                 // MirrorServer::on_disconnected(callback.connection_id).await;
        //             }
        //         }
        //     }
        // });
        ...
        if let Ok(mut serv) = Server::new(config, "0.0.0.0:7777".to_string(), Arc::new(move |callback: kcp2k_rust::kcp2k_callback::Callback| {
            ...
            // 设置服务器
            if let mut instance = MirrorServer::get_instance().lock().await {
    ...
    pub fn set_kcp_serv(&mut self, serv: Server) {
        self.kcp_serv = Some(serv);
    }

    pub async fn send(connection_id: u64, writer: &Writer, channel: kcp2k_rust::kcp2k_channel::Kcp2KChannel) {
        if let Some(mut serv) = MirrorServer::get_instance().lock().await.kcp_serv.as_mut() {
    ...
    pub async fn send_bytes(connection_id: u64, data: Vec<u8>, channel: kcp2k_rust::kcp2k_channel::Kcp2KChannel) {
        if let Some(mut serv) = MirrorServer::get_instance().lock().await.kcp_serv.as_mut() {
    ...
    pub async fn disconnect(connection_id: u64) {}
    ...
    pub async fn on_connected(con_id: u64) {
        println!("OnConnected {}", con_id);

        if let mut connections_manager = ConnectionsManager::get_instance().lock().await {
            if con_id == 0 || connections_manager.is_has_connection(con_id) {
                MirrorServer::disconnect(con_id).await;
            }
            ...
            MirrorServer::send(connection.connection_id, &writer, kcp2k_rust::kcp2k_channel::Kcp2KChannel::Reliable).await;
            connections_manager.insert_connection(connection);
        }
    ...
    pub async fn on_data(con_id: u64, message: Vec<u8>, channel: kcp2k_rust::kcp2k_channel::Kcp2KChannel) {
        if let mut connections_manager = ConnectionsManager::get_instance().lock().await {
            let mut t_reader = Reader::new_with_len(&message, true);
            if let Some(connection) = connections_manager.get_connection_mut(con_id) {
            ...
            while t_reader.get_remaining() > 0 {
                ...
                if msg_type_hash == "Mirror.TimeSnapshotMessage".get_stable_hash_code16() {
                    ...
                    // print!("{}", output_first);
                    if let Some(cur_connection) = connections_manager.get_connection(con_id) {
                        let mut writer = Writer::new_with_len(true);
                        // 写入 TimeSnapshotMessage 数据
                        TimeSnapshotMessage {}.serialization(&mut writer);
                        // 发送 writer 数据
                        MirrorServer::send(cur_connection.connection_id, &writer, kcp2k_rust::kcp2k_channel::Kcp2KChannel::Reliable).await;
                    }
                    ...
                    if let Some(cur_connection) = connections_manager.get_connection(con_id) {
                        ...
                        // 发送 writer 数据
                        MirrorServer::send(cur_connection.connection_id, &writer, channel).await;
                    }
                    ...
                    // 设置连接为准备状态
                    if let Some(cur_connection) = connections_manager.get_connection_mut(con_id) {
                    ...
                    if let Some(cur_connection) = connections_manager.get_connection(con_id) {
                        ...
                        //  通知当前客户自己和已经连接的客户端
                        for (_, connection) in connections_manager.get_connections() {
                            // 自己
                        ...
                        // 发送给当前连接
                        MirrorServer::send(cur_connection.connection_id, &cur_writer, kcp2k_rust::kcp2k_channel::Kcp2KChannel::Reliable).await;
                        ...
                        for (_, connection) in connections_manager.get_connections() {
                            ...
                            MirrorServer::send(connection.connection_id, &other_writer, kcp2k_rust::kcp2k_channel::Kcp2KChannel::Reliable).await;
                        }
                    }
    ...
    pub async fn on_disconnected(con_id: u64) {
        if let mut connections_manager = ConnectionsManager::get_instance().lock().await {
            if let Some(connection) = connections_manager.get_connection(con_id) {
                let mut writer = Writer::new_with_len(true);
                ...
                connections_manager.remove_connection(con_id);
            }
        }
    }
}